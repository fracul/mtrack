...
int main(int argc, char ** argv)
{
  ...
  if (comm.world_size != ebeam.Nbunch+1) 
  {
    fprintf(stderr, "Process number must be Nbunch + 1 (Lc = %g ; Nharm = %d ; Nbunch = %d ; processes = %d).\n", ring.Lc, ring.Nharm, ebeam.Nbunch, comm.world_size);
    MPI_Finalize();
    return -1;
  }
  ...
  switch (comm.proc_id)
  {
    ...
    case MANAGER_RANK:
      ...
      {
        manager_weakweak(ring, track, ebeam, macrop_model, &SelfFieldModel);
        ring_destroy(&ring);
        tracking_destroy(&track);
      }
      else if(track.BunchModel == MODEL_STRONG)
        manager_weakstrong(ring, track, ebeam, macrop_model);
    ...
    default:
      ...
      {
       setup_tracking_parameters(&ring, &track, &SelfFieldModel, &macrop_model);
       worker_weakweak(ring, track, ebeam, macrop_model, SelfFieldModel);
       ring_destroy(&ring);
       tracking_destroy(&track);
      }
      else if(track.BunchModel == MODEL_STRONG)
        worker_weakstrong(ring, track, ebeam, macrop_model);
      break;

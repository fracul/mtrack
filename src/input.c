                    ...
                    selffield_model_t * SelfFieldModel)
{
 ...
 if(!config_get_int(fp, section, "EnableQuantum", &(track->EnableQuantum)))
  ...
  if (config_have_section(fp,section) && enable_resonators!=0) {
  ...
  while (config_have_section(fp,section)){
    ...
    fbt_feedback_t * fbt = &(ring->fbt_feedback[i-1]);
    ...
    if (!config_get_double(fp,section,"gain",&(fbt->gain)))
      return false;
...
bool setup_tracking_parameters(ring_t * ring, tracking_t * track, selffield_model_t * SelfFieldModel, bunch_macroparticle_model_t * macrop_model)
{
  ...
  if(track->scan == 3)
    {
      if(ring->longrange_resonators_size[LON] < 1)
      {
      ...
      unsigned int i;
      for(i = 0; i < ring->longrange_resonators_size[LON]; i++)
        ring->longrange_resonators[LON][i].Qfactor = ring->longrange_resonators[LON][i].Qfactor / track->Nscan;
    }
    if(track->scan == 4)
    {
      if(ring->longrange_resonators_size[LON] < 1)
      ...
      unsigned int i;
      for(i = 0; i < ring->longrange_resonators_size[LON]; i++)
        ring->longrange_resonators[LON][i].Qfactor = ring->longrange_resonators[LON][i].Qfactor * track->scan_start;
    }
...
fprint_resonators(FILE * fp, const ring_t ring, const selffield_model_t SelfFieldModel, const bunch_macroparticle_model_t macrop_model)
{
  ...
  for(k = 0; k < ring.longrange_resonators_size[LON]; k++) 
  {
    const LR_resonator_t resonator = ring.longrange_resonators[LON][k];
...
int ring_destroy(ring_t * ring)
{
  ...
  for(i = 0; i < ring->longrange_resonators_size[LON]; i++)
  {
    LR_resonator_t * ResonImp = &ring->longrange_resonators[LON][i];
